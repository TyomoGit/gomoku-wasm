...
function initEventListeners(canvas, ctx, messageField) {
    ...
    canvas.addEventListener('click', (event) => __awaiter(this, void 0, void 0, function* () {
        if (!userCanPut) {
            return;
        }
...
function drawStone(x, y, color, ctx, opacity) {
    ...
    ctx.beginPath();
    ctx.arc(x * GRID_SIZE + GRID_SIZE * PADDING_SCALE, y * GRID_SIZE + GRID_SIZE * PADDING_SCALE, OFFSET - STONE_PADDING, 0, 2 * Math.PI);
